...
const PlayerProfileModal = ({ 
    ...
    const calculateRadarForPlayer = useCallback((targetMatches, targetTotalGames, targetTotalChips) => {
    ...
    const radarStats = useMemo(() => {
        if (!player) return [];
        return calculateRadarForPlayer(playerMatches, totalGames, player.totalChips);
    }, [player, totalGames, playerMatches, calculateRadarForPlayer]);
    ...
    const compareRadarStats = useMemo(() => {
        ...
        return calculateRadarForPlayer(targetMatches, targetObj.gamesPlayed, targetObj.totalChips);
    }, [compareTarget, history, leaderboardData, calculateRadarForPlayer]);
    ...
    const badges = useMemo(() => {
        ...
        Object.entries(seasonsMap).forEach(([seasonKey, matches]) => {
            ...
            if (totalMatchesInSeason > 0 && myMatchesInSeason === totalMatchesInSeason) {
                b.push({ icon: 'calendar-check', name: `${seasonKey} 全勤王`, color: 'text-blue-600 bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-700 dark:text-blue-400', desc: `${seasonKey} 赛季保持全勤 (${myMatchesInSeason}/${totalMatchesInSeason})` });
            }
        ...
        return b;
    }, [player, totalGames, wins, playerMatches, history]);
    ...
    const displayedMatches = [...playerMatches].reverse();

    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/80 backdrop-blur-md animate-modal" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="player-profile-title">
            <div ref={modalRef} className="glass-panel w-full max-w-4xl max-h-[95vh] overflow-y-auto rounded-2xl shadow-2xl relative bg-white/90 dark:bg-slate-900/90" onClick={e => e.stopPropagation()}>
                <div className="h-32 bg-gradient-to-r from-emerald-600 to-teal-700 dark:from-emerald-900 dark:to-slate-900 relative overflow-hidden">
                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-30" aria-hidden="true"></div>
                    <button ref={closeButtonRef} onClick={onClose} aria-label="关闭玩家档案" className="absolute top-4 right-4 p-2 rounded-full bg-black/20 text-white hover:bg-white/20 transition-colors z-10">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                        <div className="relative group cursor-pointer">
                            ...
                            <label className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-full opacity-0 group-hover:opacity-100 cursor-pointer transition-all backdrop-blur-sm">
                                <Icon name="camera" className="text-white w-8 h-8" aria-hidden="true" />
                                <span className="sr-only">上传头像</span>
                                <input type="file" accept="image/*" className="hidden" onChange={handleFileChange} aria-label={`为 ${player.name} 上传头像`} />
                            </label>
                        </div>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700/50 shadow-inner">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <div className="relative">
                                        ...
                                        <select id="compare-player-select" value={compareTarget} onChange={e => setCompareTarget(e.target.value)} aria-label="选择要对比的玩家" className="appearance-none bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs rounded px-2 py-1 pr-6 focus:outline-none focus:border-emerald-500">
                                            <option value="">⚔️ VS 对比</option>
                                            {allPlayerNames.filter(n => n !== player.name).map(n => (<option key={n} value={n}>{n}</option>))}
                                        </select>
                                        <div className="absolute inset-y-0 right-0 flex items-center px-1 pointer-events-none text-slate-400" aria-hidden="true"><Icon name="chevron-down" className="w-3 h-3"/></div>
                                    </div>
                                </div>
                                            ...
                                            <div key={i} className="flex flex-col gap-1">
                                                <div className="flex justify-between items-end text-xs">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {compareVal !== null && (<span className={`text-[10px] font-mono ${diff > 0 ? 'text-emerald-500' : diff < 0 ? 'text-rose-500' : 'text-slate-400'}`}>{diff > 0 ? '+' : ''}{diff.toFixed(1)}</span>)}
                                                ...
                                                <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden relative">
                                                    ...
                                                    {compareVal !== null && (<div className="absolute h-full bg-rose-500/50 rounded-full" style={{ width: `${compareVal * 10}%`, zIndex: 20, mixBlendMode: 'multiply' }}></div>)}
                                                </div>
